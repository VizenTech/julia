...
of `T`. Each `where` introduces a single variable, so these expressions are nested for types with
...
for the outermost variable in `A`.
...
Variables in `where` expressions can be restricted. `Array{T} where T<:Integer` refers to all arrays
whose element type is some kind of `Integer`. Variables can have both lower and upper bounds.
...
The syntax `T>:Int` also works to specify only the lower bound of a variable.

Since `where` expressions nest, variable bounds can refer to outer variables.
